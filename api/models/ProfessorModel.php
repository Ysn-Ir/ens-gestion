...
class ProfessorModel {
    ...
    try {
        $sql = "
            SELECT
                e.element_id,
                e.nom,
                m.module_id, -- C'est une bonne pratique de le sélectionner aussi
                m.code AS module_code,
                m.nom AS module_name,
                m.field_id, -- <<<--- AJOUTEZ CETTE LIGNE !
                s.nom AS semestre_nom,
                s.semestre_id, -- Bonne pratique
                aa.annee_id AS annee_academique,
                f.nom AS filiere_nom,
                eta.nom_etape AS etape_nom,
                eta.etape_id, -- Bonne pratique
                e.Ref_prof_element,
                e.Ref_prof_tp
            FROM elements e
    ...
    try {
        // --- NOUVELLE VÉRIFICATION PRIORITAIRE ---
        // 1. Vérifier si le professeur enseigne au moins un élément dans ce module.
        $stmtTeaching = $this->db->prepare("
            SELECT 1 FROM elements
            WHERE module_id = ? AND (Ref_prof_element = ? OR Ref_prof_tp = ?)
            LIMIT 1
        ");
        $stmtTeaching->execute([$moduleId, $userId, $userId]);
        if ($stmtTeaching->fetch()) {
            return true; // OUI, il enseigne dans ce module, il est autorisé.
        }
        // --- FIN DE LA NOUVELLE VÉRIFICATION ---


        // Si le professeur n'enseigne pas directement, on vérifie ses droits de chef.
        $profile = $this->getProfessorDetails($userId);
        if (!$profile || !$profile['professor_degree_role']) {
            return false; // Pas de profil ou pas de rôle de chef, donc pas autorisé.
        }
        ...
        if ($professorDegreeRole === 'Chef_de_Filiere') {
            return $this->isFieldHead($userId, $moduleInfo['field_id']);
        } elseif ($professorDegreeRole === 'Chef_de_Departement') {
            return $this->isDepartmentHead($userId, $moduleInfo['department_id']);
        }
