...
public function addProfessor($userData, $profData, $roles = []) {
    ...
    public function getAllProfessors($search = '', $limit = 100, $offset = 0)
{
    try {
        ...
        // Group by to handle multiple roles and add pagination
        $query .= " GROUP BY p.user_id, p.nom, p.prenom, p.telephone, p.department_id, p.annee_id, p.actuel, u.email, d.nom";
        ...
        // Transform roles into an array for easier handling
        ...
        error_log("getAllProfessors[search=$search, limit=$limit, offset=$offset]: " . $e->getMessage());
        throw new Exception('Failed to retrieve professors: ' . $e->getMessage());
    ...
    public function updateProfessor($userId, $profData, $roles = [])
{
    ...
    try {
        ...
        error_log("updateProfessor[userId=$userId]: Input profData: " . json_encode(array_diff_key($profData, ['password' => ''])));
        ...
        if (
            ...
            throw new Exception("Required professor data is missing (cin, nom, prenom, department_id)");
        }

        // Validate email format if provided
        if (!empty($profData['email']) && !filter_var($profData['email'], FILTER_VALIDATE_EMAIL)) {
            throw new Exception("Invalid email format");
        }

        // Validate telephone format if provided
        if (!empty($profData['telephone']) && !preg_match('/^\+?[0-9\s\-]{7,15}$/', $profData['telephone'])) {
            throw new Exception("Invalid telephone format");
        }

        // Validate date formats if provided
        if (!empty($profData['date_debut_affectation']) && !DateTime::createFromFormat('Y-m-d', $profData['date_debut_affectation'])) {
            throw new Exception("Invalid date_debut_affectation format");
        }
        if (!empty($profData['date_fin_affectation']) && !DateTime::createFromFormat('Y-m-d', $profData['date_fin_affectation'])) {
            throw new Exception("Invalid date_fin_affectation format");
        }

        // Validate department_id
        $stmt = $this->db->prepare("SELECT department_id FROM departements WHERE department_id = ?");
        $stmt->execute([$profData['department_id']]);
        if (!$stmt->fetch()) {
            throw new Exception("Invalid department_id: " . $profData['department_id']);
        }

        // Validate field_id if provided
        if (!empty($profData['field_id'])) {
            $stmt = $this->db->prepare("SELECT field_id FROM filieres WHERE field_id = ?");
            $stmt->execute([$profData['field_id']]);
            if (!$stmt->fetch()) {
                throw new Exception("Invalid field_id: " . $profData['field_id']);
            }
        }

        // Validate username format if provided
        if (!empty($profData['username'])) {
            if (!preg_match('/^[a-zA-Z0-9_]{3,50}$/', $profData['username'])) {
                throw new Exception("Invalid username format: Must be 3-50 characters, alphanumeric or underscore");
            }
            // Check username uniqueness
            $stmt = $this->db->prepare("SELECT user_id FROM utilisateurs WHERE username = ? AND user_id != ?");
            $stmt->execute([$profData['username'], $userId]);
            if ($stmt->fetch()) {
                throw new Exception("Username already used by another user: " . $profData['username']);
            }
        }
        ...
        $stmt = $this->db->prepare("SELECT user_id, username FROM utilisateurs WHERE user_id = ?");
        $stmt->execute([$userId]);
        ...
        $stmt = $this->db->prepare("SELECT user_id FROM professeurs WHERE user_id = ?");
        $stmt->execute([$userId]);
        if (!$stmt->fetch()) {
        ...
        if (empty($anneeId)) {
            ...
            // Validate provided annee_id
            $stmt = $this->db->prepare("SELECT annee_id FROM annees_academiques WHERE annee_id = ?");
            $stmt->execute([$anneeId]);
            if (!$stmt->fetch()) {
                throw new Exception("Invalid annee_id: " . $anneeId);
            }
        }

        // Role mapping for professor_roles and utilisateurs
        $roleMapping = [
            'prof' => 'Regular',
            'chef_dep' => 'Chef_de_Departement',
            'chef_fill' => 'Chef_de_Filiere',
            ...
            'chef_element' => 'Chef_de_Element',
            'Regular' => 'prof',
            'Chef_de_Departement' => 'chef_dep',
            'Chef_de_Filiere' => 'chef_fill',
            'Chef_de_Module' => 'chef_module',
            'Chef_de_Element' => 'chef_element'
        ];
        $validUtilisateurRoles = ['student', 'admin', 'prof', 'chef_dep', 'chef_fill', 'moderator', 'superadmin'];

        // Normalize role input
        $inputRole = !empty($profData['role']) ? trim($profData['role']) : 'prof';
        error_log("updateProfessor[userId=$userId]: Input role: " . $inputRole);
        $utilisateurRole = isset($roleMapping[$inputRole]) && in_array($roleMapping[$inputRole], $validUtilisateurRoles)
            ? $roleMapping[$inputRole]
            : (in_array($inputRole, $validUtilisateurRoles) ? $inputRole : null);
        if ($utilisateurRole === null) {
            throw new Exception("Invalid primary role for utilisateurs table: " . $inputRole . ". Valid roles: " . implode(', ', $validUtilisateurRoles));
        }

        // Validate additional roles
        foreach ($roles as $role) {
            if (!isset($roleMapping[$role])) {
                throw new Exception("Invalid additional role: " . $role . ". Valid roles: " . implode(', ', array_keys($roleMapping)));
            }
        }
        ...
        if (!empty($profData['email'])) {
            $userSqlParts[] = "email = ?";
            $userParams[] = $profData['email'];
        }
        if (!empty($profData['username'])) {
            $userSqlParts[] = "username = ?";
            $userParams[] = $profData['username'];
        }
        if (!empty($profData['password'])) {
            $userSqlParts[] = "password_hash = ?";
            $userParams[] = password_hash($profData['password'], PASSWORD_BCRYPT);
        }
        if (!empty($profData['role'])) {
            $userSqlParts[] = "role = ?";
            $userParams[] = $utilisateurRole;
        }
        if (isset($profData['actuel'])) {
            $userSqlParts[] = "actuel = ?";
            $userParams[] = $profData['actuel'];
        }

        $utilisateursUpdated = false;
        if (!empty($userSqlParts)) {
            $userSql = "UPDATE utilisateurs SET " . implode(", ", $userSqlParts) . " WHERE user_id = ?";
            $userParams[] = $userId;
            error_log("updateProfessor[userId=$userId]: Executing utilisateurs query: $userSql with params: " . json_encode($userParams));
            $stmt = $this->db->prepare($userSql);
            $stmt->execute($userParams);
            $utilisateursUpdated = $stmt->rowCount() > 0;
            if (!$utilisateursUpdated) {
                error_log("updateProfessor[userId=$userId]: No rows affected in utilisateurs table. Current username: " . $currentUser['username'] . ", Provided username: " . ($profData['username'] ?? 'not provided'));
            }
        } else {
            error_log("updateProfessor[userId=$userId]: No fields provided for utilisateurs update");
        }

        // Mise à jour de la table professeurs
        $stmt = $this->db->prepare("
            UPDATE professeurs
            SET cin = ?, nom = ?, prenom = ?, telephone = ?, department_id = ?, annee_id = ?, actuel = ?, rapport = ?
            WHERE user_id = ?
        ");
        $profParams = [
            $profData['cin'],
            ...
            $anneeId,
            $profData['actuel'] ?? 1,
            $profData['rapport'] ?? null,
            $userId
        ];
        error_log("updateProfessor[userId=$userId]: Executing professeurs query with params: " . json_encode($profParams));
        $stmt->execute($profParams);
        if ($stmt->rowCount() === 0) {
            error_log("updateProfessor[userId=$userId]: No rows affected in professeurs table");
        }

        // Mise à jour du chef de département, si applicable
        if ($utilisateurRole === 'chef_dep') {
            $stmt = $this->db->prepare("SELECT head_professor_id FROM departements WHERE department_id = ?");
            $stmt->execute([$profData['department_id']]);
            $oldChef = $stmt->fetchColumn();

            if ($oldChef && $oldChef != $userId) {
                $stmt = $this->db->prepare("
            ...
            $stmt = $this->db->prepare("
                UPDATE departements 
                SET head_professor_id = ?
                WHERE department_id = ? AND head_professor_id != ?
            ");
            $stmt->execute([$userId, $profData['department_id'], $userId]);
        } else {
            $stmt = $this->db->prepare("
                UPDATE departements 
                SET head_professor_id = NULL 
                WHERE head_professor_id = ?
            ");
            $stmt->execute([$userId]);
        }

        // Mise à jour du chef de filière, si applicable
        if ($utilisateurRole === 'chef_fill' && !empty($profData['field_id'])) {
            $stmt = $this->db->prepare("SELECT head_professor_id FROM filieres WHERE field_id = ?");
            $stmt->execute([$profData['field_id']]);
            $oldChef = $stmt->fetchColumn();
            ...
            ");
            $stmt->execute([$userId, $profData['field_id'], $userId]);
        } else {
            $stmt = $this->db->prepare("
                UPDATE filieres 
                SET head_professor_id = NULL 
        ...
        $primaryRole = $roleMapping[$utilisateurRole] ?? 'Regular';
        $stmt = $this->db->prepare("
            INSERT INTO professor_roles (user_id, role, date_role, date_debut_affectation, date_fin_affectation)
            VALUES (?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            ...
            $primaryRole,
            date('Y-m-d H:i:s'),
            $profData['date_debut_affectation'] ?? null,
            $profData['date_fin_affectation'] ?? null
        ]);

        // Insérer les rôles supplémentaires
        foreach ($roles as $role) {
            if (isset($roleMapping[$role])) {
                $stmt->execute([
                    $userId,
        ...
        return [
            'status' => 200,
            'data' => [
                'message' => 'Professor updated successfully' . ($utilisateursUpdated ? '' : ' (no changes to utilisateurs table)'),
                'user_id' => $userId
            ]
        ];
    } catch (PDOException $e) {
        $this->db->rollBack();
        error_log("updateProfessor[userId=$userId]: SQL Error - " . $e->getMessage() . " | Input: " . json_encode(array_diff_key($profData, ['password' => ''])));
        return [
            'status' => 500,
            'data' => ['message' => 'SQL Error: ' . $e->getMessage()]
        ];
        ...
        $this->db->rollBack();
        error_log("updateProfessor[userId=$userId]: Application Error - " . $e->getMessage() . " | Input: " . json_encode(array_diff_key($profData, ['password' => ''])));
        return [
            'status' => 400,
            'data' => ['message' => 'Application Error: ' . $e->getMessage()]
        ];
