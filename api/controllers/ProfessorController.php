...
/**
 * Exporte les notes d'un module pour une filière donnée au format CSV.
 * @param int $moduleId L'ID du module.
 * @param int $fieldId L'ID de la filière.
 */
public function exportModuleNotes($moduleId, $fieldId) {
    try {
        (new ProfessorMiddleware())->verifyProfessor();
        $userId = $_SESSION['user']['user_id'];

        // --- Étape 1: Vérification des autorisations ---
        // Un professeur peut exporter s'il est chef de la filière, chef du département
        // ou s'il enseigne au moins un élément dans ce module.
        $isAuthorized = false;
        if ($this->model->canAccessModuleNotes($userId, $moduleId)) {
            $isAuthorized = true;
        }
        ...
        if (!$isAuthorized) {
            $this->response->send(403, ['error' => 'Vous n\'êtes pas autorisé à exporter les notes de ce module.']);
            return;
        }

        // --- Étape 2: Récupération des données ---
        // On peut réutiliser la fonction getModuleNotes car elle contient toutes les infos nécessaires.
        $notes = $this->model->getModuleNotes($moduleId);

        if (empty($notes)) {
            // Gérer le cas où il n'y a pas de notes à exporter
            // On pourrait afficher une page HTML simple ou un message.
            echo "Aucune note à exporter pour ce module.";
            return;
        }

        // --- Étape 3: Génération du fichier CSV ---
        $moduleName = $notes[0]['module_name']; // Nom du module pour le nom du fichier
        $fileName = "Export-Notes-" . preg_replace('/[^a-zA-Z0-9_-]/', '-', $moduleName) . ".csv";

        // Définir les en-têtes HTTP pour forcer le téléchargement
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="' . $fileName . '"');

        // Ouvrir le flux de sortie PHP pour y écrire le CSV
        ...
        // Écrire chaque ligne de note dans le fichier
        foreach ($notes as $note) {
            fputcsv($output, [
                $note['student_nom'] . ' ' . $note['student_prenom'],
                $note['element_name'],
                $note['note_tp'] ?? '', // Utilise une chaîne vide si la note est null
                $note['note_td'] ?? '',
                $note['note_cc'] ?? '',
                $note['note_exam'] ?? '',
        ...
        error_log("Error in exportModuleNotes: " . $e->getMessage());
        // En cas d'erreur, il est difficile d'envoyer une réponse JSON car les en-têtes sont déjà modifiés.
        // On peut se contenter de logger l'erreur.
        http_response_code(500 );
        echo "Une erreur interne est survenue lors de la génération de l'export.";
    }
}
