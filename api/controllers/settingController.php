...
class settingController {
    ...
    public function getAllConstraints()
    {
        try {
            ...
            $constraints = $this->model->getAllConstraints($scope);
    ...
    public function getAllAcademicYears()
    {
        try {
            ...
            $academicYears = $this->model->getAllAcademicYears();
            $this->response->sendSuccess($academicYears, 'Academic years retrieved successfully');
        } catch (PDOException $e) {
            $this->response->sendError('Database error: ' . $e->getMessage(), 500);
        } catch (Exception $e) {
            $this->response->sendError($e->getMessage(), $e->getCode() ?: 500);
    ...
    public function addAcademicYear()
    {
        try {
            ...
            $input = json_decode(file_get_contents('php://input'), true);
            if (!isset($input['annee_id']) || !isset($input['current_flag'])) {
                throw new Exception('Missing required fields', 400);
            }

            $success = $this->model->addAcademicYear($input['annee_id'], $input['current_flag']);
            if ($success) {
                $this->response->sendSuccess([], 'Academic year added successfully', 201);
            } else {
                throw new Exception('Failed to add academic year', 500);
            }
        } catch (Exception $e) {
            $this->response->sendError($e->getMessage(), $e->getCode() ?: 500);
        }
    ...
    public function updateAcademicYear()
    {
        try {
            ...
            $success = $this->model->updateAcademicYear($input['annee_id'], $input['current_flag']);
            if ($success) {
                $this->response->sendSuccess([], 'Academic year updated successfully');
            } else {
                throw new Exception('Failed to update academic year', 500);
            }
        } catch (Exception $e) {
            $this->response->sendError($e->getMessage(), $e->getCode() ?: 500);
        }
    ...
    public function deleteAcademicYear()
    {
        try {
            ...
            $input = json_decode(file_get_contents('php://input'), true);
            if (!isset($input['annee_id'])) {
                throw new Exception('Missing annee_id', 400);
            }

            $success = $this->model->deleteAcademicYear($input['annee_id']);
            if ($success) {
                $this->response->sendSuccess([], 'Academic year deleted successfully');
            } else {
                throw new Exception('Failed to delete academic year', 500);
            }
        } catch (Exception $e) {
            $this->response->sendError($e->getMessage(), $e->getCode() ?: 500);
        }
